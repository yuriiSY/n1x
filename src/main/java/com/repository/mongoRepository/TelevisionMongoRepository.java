...
public class TelevisionMongoRepository implements CrudRepository<Tablet> {
    private final MongoCollection<Document> collection;
    private final Gson gson;

    public TelevisionMongoRepository() {
        gson = new GsonBuilder().registerTypeAdapter(LocalDateTime.class,
                ...
                .create();
        collection = Mongo.getDatabase().getCollection("television");
    }
    ...
    public void saveAll(List<Tablet> products) {
        ...
        for (Tablet tablet : products) {
            documents.add(Document.parse(gson.toJson(tablet)));
        }
